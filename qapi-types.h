...
typedef enum MigrationStatus
{
    ...
    MIGRATION_STATUS_CHECKPOINTING = 5,
...
typedef struct MCStats MCStats;

typedef struct MCStatsList
{
    union {
        MCStats *value;
        uint64_t padding;
    };
    struct MCStatsList *next;
} MCStatsList;
...
typedef enum MigrationCapability
{
    ...
    MIGRATION_CAPABILITY_MC = 4,
    MIGRATION_CAPABILITY_MC_NET_DISABLE = 5,
    MIGRATION_CAPABILITY_MC_RDMA_COPY = 6,
    MIGRATION_CAPABILITY_RDMA_KEEPALIVE = 7,
    MIGRATION_CAPABILITY_MAX = 8,
} MigrationCapability;
