...
+
+#ifdef DEBUG_MC
+#define DPRINTF(fmt, ...) +    do { printf("mc: " fmt, ## __VA_ARGS__); } while (0)
+#else
+#define DPRINTF(fmt, ...) +    do { } while (0)
+#endif
+
+#ifdef DEBUG_MC_VERBOSE
+#define DDPRINTF(fmt, ...) +    do { printf("mc: " fmt, ## __VA_ARGS__); } while (0)
+#else
+#define DDPRINTF(fmt, ...) +    do { } while (0)
+#endif
+
+#ifdef DEBUG_MC_REALLY_VERBOSE
+#define DDDPRINTF(fmt, ...) +    do { printf("mc: " fmt, ## __VA_ARGS__); } while (0)
+#else
...
+#define CALC_MAX_STRIKES()                                           +    do {  max_strikes = (max_strikes_delay_secs * 1000) / freq_ms; } +    while (0)
+
+/*
...
+
+#define SLAB_RESET(s) do {                      +                            s->size = 0;      +                            s->read = 0;      +                      } while(0)
+
...
+        fprintf(stderr, "Unable to control qdisc: %s! %p %p %dn",
...
+       fprintf(stderr, "MC: Unable to change buffer size: %sn",
+			nl_geterror(err));
+       return -EINVAL;
+    }
+
+    DPRINTF("Set buffer size to %d bytesn", size);
+
+    return mc_deliver(1);
+}
+
...
+
+    if (first_nic_chosen) {
+         fprintf(stderr, "Micro-Checkpointing with multiple NICs not yet supported!n");
+         return;
+    }
+
+    if (!nc->peer) {
+        fprintf(stderr, "Micro-Checkpoint nic %s does not have peer host device for buffering. VM will not be consistent.n", nc->name);
+        return;
+    }
+
+    name = nc->peer->info_str;
+
+    DPRINTF("Checking contents of device [%s] (%s)n", name, nc->name);
+
+    if (strncmp(name, key, keylen)) {
+        fprintf(stderr, "Micro-Checkpoint nic %s does not have 'ifname' "
+                        "in its description (%s, %s). Trying workaround...n",
+                        nc->name, name, nc->peer->name);
+        key = "fd=";
+        keylen = strlen(key);
+        if (strncmp(name, key, keylen)) {
+            fprintf(stderr, "Still cannot find 'fd=' either. Failure.n");
+            return;
+        }
+
+        use_fd = true;
...
+    if (use_fd) {
+        struct ifreq r;
+        DPRINTF("Want to retreive name from fd: %dn", atoi(device));
+
+        if (ioctl(atoi(device), TUNGETIFF, &r) == -1) {
+            fprintf(stderr, "Failed to convert fd %s to name.n", device);
+            return;
+        }
+
+        DPRINTF("Got name %s!n", r.ifr_name);
...
+        fprintf(stderr, "MC: Unable to release indefinite: %sn",
+            nl_geterror(err));
+        return -EINVAL;
+    }
+
+    DPRINTF("Buffering suspendedn");
+
+    return mc_deliver(1);
+}
+
...
+        fprintf(stderr, "Unable to release indefinite: %sn", nl_geterror(err));
+    }
+
+out:
...
+
+    if (buffering_enabled) {
+        fprintf(stderr, "Buffering already enable Skipping.n");
+        return 0;
+    }
+
+    first_nic_chosen = 0;
...
+
+    fprintf(stderr, "Initializing buffering for nic %s => %sn", dev, buffer_dev);
+
+    if (sock == NULL) {
+        sock = (struct nl_sock *) nl_cli_alloc_socket();
+        if (!sock) {
+            fprintf(stderr, "MC: failed to allocate netlink socketn");
+            goto failed;
+        }
...
+        if (!qdisc) {
+            fprintf(stderr, "MC: failed to allocate netlink qdiscn");
+            goto failed;
+        }
...
+        if (!link_cache) {
+            fprintf(stderr, "MC: failed to allocate netlink link_cachen");
+            goto failed;
+        }
+    }
+
...
+        fprintf(stderr, "Qdisc device '%s' does not exist!n", buffer_dev);
+        goto failed;
+    }
+
+    if (!rtnl_tc_get_parent(tc)) {
+        fprintf(stderr, "Qdisc parent '%s' is not valid!n", parent);
+        goto failed;
+    }
+
+    if (rtnl_tc_set_kind(tc, "plug") < 0) {
+        fprintf(stderr, "Could not open qdisc plug!n");
+        goto failed;
+    }
+
...
+        fprintf(stderr, "Qdisc plug not supported!n");
+        goto failed;
+    }
+
...
+
+    if (mc_deliver(0) < 0) {
+		fprintf(stderr, "First time qdisc create failedn");
+		goto failed;
+    }
+
+    DPRINTF("Buffering enabled, size: %d MB.n", buffer_size / 1024 / 1024);
+
...
+        DPRINTF("MC setting new buffer size to %dn", buffer_size);
+        if (mc_set_buffer_size(buffer_size) < 0)
+            return -EINVAL;
+    }
+
+    if ((err = rtnl_qdisc_plug_buffer((void *) qdisc)) < 0) {
+        fprintf(stderr, "Unable to flush oldest checkpoint: %sn", nl_geterror(err));
+        return -EINVAL;
+    }
+
+    DDPRINTF("Inserted checkpoint barriern");
+
+    return mc_deliver(1);
+}
+
...
+    }
+
+    DDPRINTF("Flushed oldest checkpoint barriern");
+
+    return mc_deliver(1);
+}
+
...
+        DDPRINTF("Found copyset slab @ idx %dn", slab->idx);
+        mc->mem_slab = slab;
+    }
+
+    return slab;
...
+
+        if (put == 0) {
+            DDPRINTF("Reached the end of slab %d Need a new onen", slab->idx);
+            goto zero;
+        }
+
...
+                                        put);
+
+            DDDPRINTF("Attempted offloaded memcpy.n");
+
+            if (ret != RAM_COPY_CONTROL_NOT_SUPP) {
+                if (ret == RAM_COPY_CONTROL_DELAYED) {
+                    DDDPRINTF("Offloaded memcpy successful.n");
+                    mc->copy->offset += put;
+                    goto next;
+                } else {
+                    fprintf(stderr, "Offloaded memcpy failed: %dn", ret);
+                    return ret;
+                }
+            }
+        }
+
+        DDDPRINTF("Copying to %p from %p, size %" PRId64 "n",
...
+                  " total %" PRIu64 " size: %" PRIu64 " slab %dn",
...
+
+        DDDPRINTF("copyset %d copies: %" PRIu64 " total: %" PRIu64 "n",
...
+                fprintf(stderr, "Failure to initiate copyset %d index %dn",
+                        copyset->idx, idx);
+                migrate_set_state(s, MIG_STATE_CHECKPOINTING, MIG_STATE_ERROR);
+                vm_start();
+                goto out;
+            }
+
+            DDDPRINTF("Success copyset %d index %dn", copyset->idx, idx);
+        }
+
+        copyset->nb_copies = 0;
+    }
+
+    DDPRINTF("Copy complete.n");
+
+skip_copies:
...
+                "bailing: %sn", request, strerror(-ret));
+    } else {
+        DDPRINTF("transaction: sent: %s (%" PRIu64 ")n",
...
+            mc->slab_strikes++;
+            DDPRINTF("MC has strike %d slabs %" PRIu64 " max %dn",
...
+
+    if (mc->slab_strikes) {
+        DDPRINTF("MC used all slabs. Resetting strikes to zero.n");
+        mc->slab_strikes = 0;
+    }
+skip:
+
...
+
+            DPRINTF("MC has reached max strikes. Will free %d / %d copies max %dn",
...
+            mc->copy_strikes++;
+            DDPRINTF("MC has strike %d copies %d max %dn",
...
+
+    if (mc->copy_strikes) {
+        DDPRINTF("MC used all copies. Resetting strikes to zero.n");
+        mc->copy_strikes = 0;
+    }
+skip:
+
...
+        fprintf(stderr, "Failed to setup read MC controln");
+        goto err;
+    }
+
+    if (!(mc_staging = qemu_fopen_mc(&mc, "wb"))) {
+        fprintf(stderr, "Failed to setup MC staging arean");
+        goto err;
+    }
+
...
+            fprintf(stderr, "transaction start failedn");
+            break;
+        }
+
...
+                fprintf(stderr, "failed 1, skipping sendn");
+                goto err;
+            }
+
+            if (qemu_file_get_error(s->file)) {
+                fprintf(stderr, "failed 2, skipping sendn");
+                goto err;
+            }
+
+            DDPRINTF("Sent idx %d slab size %" PRId64 " all %ldn",
...
+
+        if (commit_sent) {
+            DDPRINTF("Waiting for commit ACKn");
+
...
+        ret = qemu_file_get_error(s->file);
+        if (ret) {
+            fprintf(stderr, "Error sending checkpoint: %dn", ret);
+            goto err;
+        }
+
+        DDPRINTF("Memory transfer complete.n");
+
+        /*
...
+
+    if (!mc_requested) {
+        DPRINTF("Source has not requested MC. Returning.n");
+        return;
+    }
+
+    if (!(mc_control = qemu_fopen_socket(fd, "wb"))) {
+        fprintf(stderr, "Could not make incoming MC control channeln");
+        goto rollback;
+    }
+
+    if (!(mc_staging = qemu_fopen_mc(&mc, "rb"))) {
+        fprintf(stderr, "Could not make outgoing MC staging arean");
+        goto rollback;
+    }
+
+    //qemu_set_block(fd);
...
+                sizeof(timeout)) < 0)
+        printf("error : setsockopt failedn");
+
...
+                     " copyset start: %" PRIu64 " slabs %" PRIu64 "n",
...
+
+                DDPRINTF("Expecting size: %" PRIu64 "n", slab->size);
+
+                while (total != slab->size) {
+                    got = qemu_get_buffer(f, slab->buf + total, slab->size - total);
+                    if (got <= 0) {
+                        fprintf(stderr, "Error pre-filling checkpoint: %dn", got);
+                        goto rollback;
+                    }
+                    DDPRINTF("Received %d slab %d / %ld received %d total %"
+                             PRIu64 "n", got, total, slab->size,
...
+            }
+
+            DDPRINTF("Acknowledging successful commitn");
+
...
+            slab = mc_slab_start(&mc);
+
+            DDPRINTF("Pre-populating slabs %" PRIu64 "...n", slabs);
+
+            for(x = 1; x < slabs; x++) {
+                slab = mc_slab_next(&mc, slab);
+            }
...
+            break;
+        default:
+            fprintf(stderr, "Unknown MC action: %" PRIu64 "n", action);
+            goto rollback;
+        }
+
...
+
+            DDPRINTF("Committed Loading MC state n");
+
+            mc_copy_start(&mc);
+
+            if (qemu_loadvm_state(mc_staging) < 0) {
+                fprintf(stderr, "loadvm transaction failedn");
+                /*
...
+            mc.slab_total = checkpoint_size;
+
+            DDPRINTF("Transaction complete.n");
+            mc.checkpoints++;
+        }
+    }
+
+rollback:
+    fprintf(stderr, "MC: checkpointing stopped. Recovering VMn");
+    goto out;
+err:
+    fprintf(stderr, "Micro Checkpointing Protocol Failedn");
+    exit(1);
+out:
+    if (mc_staging) {
+        qemu_fclose(mc_staging);
...
+    assert(slab);
+
+    DDDPRINTF("got request for %d bytes %p %p. idx %dn",
...
+
+    *curr_slab = slab;
+    DDDPRINTF("Returning %" PRIu64 " / %d bytesn", size - len, size);
+
+    return size - len;
+}
...
+    MCParams *mc = opaque;
+
+    DDDPRINTF("Loading page into %p of size %" PRIu64 "n", host_addr, size);
+
...
+
+    for (i = 0; i < iovcnt; i++) {
+        DDDPRINTF("iov # %d, len: %" PRId64 "n", i, iov[i].iov_len);
...
+            "resetting strikes to %d based on a %d sec delay.n",
